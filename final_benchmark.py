...
#   2. STRATIFIED ANALYSIS: Performance is broken down by the factors that
#      matter clinically: Arterial Transit Time, physiological condition, and SNR.
#   3. SUPERIOR METRICS: In addition to standard metrics (MAE, RMSE, Bias),
#      this script calculates Median Absolute Error (MedAE) and Coefficient
#      of Variation (CoV) to provide a robust and outlier-resistant assessment.
#   4. DIRECT COMPARISON: Generates plots directly comparable to Figure 3 in the
#      Xu et al. MULTIVERSE paper, creating a clear "Our Method vs. SOTA" narrative.
...
def calculate_metrics(df_group):
    ...
    # --- Define Clinical Tolerances --- # <-- NEW
    ...
    for model in ['ls', 'nn']:
        for param in ['cbf', 'att']:
            ...
            if valid_preds.empty:
                ...
                metrics[f'{model.upper()} {param.upper()} Acc %'] = np.nan # <-- NEW
                continue
            ...
            # --- Calculate Accuracy within Tolerance --- # <-- NEW
...
def main(model_dir: Path, output_dir: Path):
    ...
    for _ in tqdm(range(NUM_SAMPLES), desc="Generating & Evaluating Samples"):
        ...
        # --- A) Evaluate with Conventional LS Fitting ---
    ...
    df_results.to_csv(output_dir / 'full_benchmark_results.csv', index=False)
    ...
    overall_summary.to_frame().T.to_csv(output_dir / 'summary_overall.csv')
    ...
    att_summary.to_csv(output_dir / 'summary_by_att.csv')
    ...
    noise_summary.to_csv(output_dir / 'summary_by_tsnr.csv')
    ...
    plot_params = {
        'ls': {'label': 'Conventional LS', 'color': 'orangered', 'linestyle': '--'},
    ...
    fig_path = output_dir / 'figure_performance_vs_att.png'
