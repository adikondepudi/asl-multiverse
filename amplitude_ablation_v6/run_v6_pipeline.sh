#!/bin/bash
#SBATCH --job-name=v6-orchestrator
#SBATCH --partition=cpu
#SBATCH --nodes=1
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --time=1:00:00
#SBATCH --output=amplitude_ablation_v6/orchestrator_%j.out
#SBATCH --error=amplitude_ablation_v6/orchestrator_%j.err

# =============================================================================
# V6 PIPELINE: Fix CBF Bias, PLD Mismatch, and NLL Cowardice
# =============================================================================
# Changes from V5:
#   1. L1 loss (reverted from NLL — NLL inflates log_var to avoid high CBF)
#   2. variance_weight: 0.5 (stronger anti-collapse penalty)
#   3. 5 PLDs [500..2500] matching in-vivo protocol (eliminates zero-padding)
#   4. Expanded CBF range: GM 10-200 (was 120), tumor_hyper 100-250 (was 200)
#   5. Expanded ATT range: GM 500-3000 (was 2500), WM 800-3500 (was 3000)
#   6. CBF clip raised to 250 (was 200)
#
# Pipeline:
#   1. Generate spatial training data (100k samples, 5 PLDs, expanded ranges)
...
#
# Usage:
#   sbatch amplitude_ablation_v6/run_v6_pipeline.sh               # Full pipeline
#   sbatch amplitude_ablation_v6/run_v6_pipeline.sh --train-only   # Skip data gen
#   sbatch amplitude_ablation_v6/run_v6_pipeline.sh --plots-only   # Skip data+training
#
...
echo "============================================"
echo "V6 PIPELINE: Fix CBF Bias, PLD Mismatch, NLL Cowardice"
echo "============================================"
echo "Started: $(date)"
...
if [ "$TRAIN_ONLY" = false ] && [ "$PLOTS_ONLY" = false ]; then
    echo ""
    echo "[1/4] Submitting data generation (100k spatial, 5 PLDs, expanded ranges)..."

    DATA_JOB=$(sbatch --parsable amplitude_ablation_v6/generate_data.slurm)
...
if [ "$PLOTS_ONLY" = false ]; then
    ...
    JOB_A=$(sbatch --parsable ${TRAIN_DEP} amplitude_ablation_v6/A_Baseline_SpatialASL/run.slurm)
    echo "  A_Baseline_SpatialASL -> Job ${JOB_A}"

    JOB_B=$(sbatch --parsable ${TRAIN_DEP} amplitude_ablation_v6/B_AmplitudeAware/run.slurm)
    ...
    for exp in A_Baseline_SpatialASL B_AmplitudeAware; do
        MODEL_DIR="amplitude_ablation_v6/${exp}/trained_models"
...
PLOT_JOB=$(sbatch --parsable ${PLOT_DEP} amplitude_ablation_v6/generate_plots.slurm)
...
echo "  Models A: amplitude_ablation_v6/A_Baseline_SpatialASL/trained_models/"
echo "  Models B: amplitude_ablation_v6/B_AmplitudeAware/trained_models/"
echo "  Plots:    bias_cov_results_v6/"
echo "============================================"
